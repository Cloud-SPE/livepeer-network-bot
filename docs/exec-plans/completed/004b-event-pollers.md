# Exec plan 004b — Reward + delegator pollers, DM sender, auto-unsub
...
| `src/domains/scheduler/reward_poller.rs` | Tick: ingest new `Reward` events → dispatch DMs to each subscriber → mark sent. Auto-unsub after `DM_FAILURE_AUTO_UNSUB` consecutive 403s. |
...
- `src/domains/subscriptions/repo.rs` — `increment_dm_failure(user, orch) -> i64` and `clear_dm_failure(user, orch)` for the auto-unsub counter.
...
|---|---|---|---|
| Reward (per-event DM) | Within ~1 poll interval of explorer surfacing the event | `DmsClosed` increments counter; auto-removes subscription at threshold | Logged, event marked sent (no per-event retry to avoid duplicate delivery to subscribers that already received it) |
...
4. Subscribing with DMs disabled → after 3 reward events, the subscription is auto-removed (visible in SQLite + logs)
